...
class PatchExpand_2(nn.Module):
    ...
    def forward(self, x):
        x = x.permute(0,2,3,1) #8*8*8*64
        x = self.expand(x) #8*8*8*128
        B, H, W, C = x.shape
        x = rearrange(x, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=2, p2=2, c=C//4)
        x= self.norm(x)
        x = x.permute(0,3,1,2) #8,32,16,16 8,64,16,16
        return x
...
class Decoder(nn.Module):
    ...
    def forward(self, coarse0_fea, coarse1_fea, fine0_fea, def_device):
        ...
        # Fill in odd columns with A and even columns with B
        ct_tensor_12[:, :, :, ::3] = coarse0_fea_1  # Odd columns
        ct_tensor_12[:, :, :, 1::3] = coarse1_fea_1  # Even columns
        ct_tensor_12[:, :, :, 2::3] = fine0_fea_1  # Even columns
...
class CNNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(in_channels=48, out_channels=6, kernel_size=1)
